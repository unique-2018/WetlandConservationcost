                   ...
                   sidebarPanel(                                                          
                     ...
                     column(12,                                                             # The input-widgets are defined in column orientation in the sidebarPanel  
                            fluidRow(                                                        # The fluidRow functions creates a row orientation in the App
                              ...
                              tags$h4(tags$em("Analyse changes in App outputs (landscape analysis) for changes in main variables.")),
                              dropdownButton(
                                selectInput("change_yd1", p("Do you want to change crop yields in the field?", 
                                                           style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                                selectInput("change_pr1", p("Do you want to change crop prices in the field?", 
                                                           style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                                selectInput("change_pc1", p("Do you want to change production costs in the field?", 
                                                           style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                                selectInput("change_dc1", p("Do you want to change drainage costs in the field?", 
                                                           style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                                
                                selectInput("direction_change1", p("What is the direction of change?", 
                                                                  style ="color: black"), choices = c("increase", "decrease"), selected = "increase"),
                                
                                sliderInput("percenchange1", p("What is the percentage increase or decrease?", style ="color: black"), 
               ...
               fluidRow(
                 ...
                 sidebarPanel(
                   ...
                   column(12,
                          ...
                          tags$h4(tags$em("Proportion of Class A, Class B and Class A or B wetlands in the landscape", style = "color:black")),
                          dropdownButton(
                            
                            sliderInput("prop_A", p("Proportion of class A wetlands in the landscape:", 
                                                    style ="color: black"), min = 0, max = 1, value = 0.6, step = 0.2),
                            sliderInput("prop_B", p("Proportion of class B wetlands in the landscape:", 
                                                    style ="color: black"), min = 0, max = 1, value = 0.6, step = 0.2),
                            sliderInput("prop_AB", p("Proportion of class A or B wetlands in the landscape:", 
                                                     style ="color: black"), min = 0, max = 1, value = 0.6, step = 0.2)
                          ),
                          
                          tags$h4(tags$em("Analyse changes in App outputs (landscape analysis) for changes in main variables..", style = "color:black")),
                          dropdownButton(
                              selectInput("change_yd", p("Do you want to change crop yields in the landscape?", 
                                                  style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                              selectInput("change_pr", p("Do you want to change crop prices in the landscape?", 
                                                         style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                              selectInput("change_pc", p("Do you want to change production costs in the landscape?", 
                                                         style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                              selectInput("change_dc", p("Do you want to change drainage costs in the landscape?", 
                                                         style ="color: black"), choices = c("No", "Yes"), selected = "No"),
                              
                              selectInput("direction_change", p("What is the direction of change?", 
                                                         style ="color: black"), choices = c("increase", "decrease"), selected = "increase"),
                              
                              sliderInput("percenchange", p("What is the percentage increase or decrease?", style ="color: black"), 
                                          value = 0, min = 0, max = 100, step = 25)
...
server <- function(input, output) {                                                   
  ...
  df_farmacre_wl <- reactive({
    ...
    editable_fl()()%>%                                                             # the editable_fl()() is a reactive table that is the product of user-selected crop production information
      dplyr::mutate(
        #post processing part: 3 cases: if there is change in yield (change_yd=1) and change is positive (=1), drainage_cost is multiplied by the percentage change
        #the variables, change_yd, direction_change and percenchange are already defined in the App section.                  
        change_yd = ifelse(input$change_yd1 == "Yes", 1, 0),
        change_pr = ifelse(input$change_pr1 == "Yes", 1, 0),
        change_pc = ifelse(input$change_pc1 == "Yes", 1, 0),
        change_dc = ifelse(input$change_dc1 == "Yes", 1, 0),
        direction_change = ifelse(input$direction_change1 == "increase", 1, 0),
        percenchange = ifelse(direction_change ==1, (100+input$percenchange1)/100, (100-input$percenchange1)/100),
        prod_cost = other_variable_cost + fixed_cost + fertilizer_cost + herb_insect_fung,  # total production cost
        prod_cost1 = case_when(
          change_pc==1 & direction_change ==1 ~ prod_cost * (100+input$percenchange1)/100, 
          change_pc==1 & direction_change ==0 ~ prod_cost * (100-input$percenchange1)/100,
          change_pc==0  ~ prod_cost),
        avg_crop_price1 = case_when(
          change_pr==1 & direction_change ==1 ~ avg_crop_price * (100+input$percenchange1)/100, 
          change_pr==1 & direction_change ==0 ~ avg_crop_price * (100-input$percenchange1)/100,
          change_pr==0  ~ avg_crop_price),
        avg_yield1 = case_when(
          change_yd==1 & direction_change ==1 ~ avg_yield * (100+input$percenchange1)/100, 
          change_yd==1 & direction_change ==0 ~ avg_yield * (100-input$percenchange1)/100,
          change_yd==0  ~ avg_yield),
        production_cost_sim = case_when(
        ...
        production_cost_sim1 = case_when(
          change_pc==1 & direction_change ==1 ~ production_cost_sim * (100+input$percenchange1)/100, 
          change_pc==1 & direction_change ==0 ~ production_cost_sim * (100-input$percenchange1)/100,
          change_pc==0  ~ production_cost_sim),
        
        # ________________Wetland Variables
        ...
        dr_cost1 = case_when(
          change_dc==1 & direction_change ==1 ~ dr_cost * (100+input$percenchange1)/100, 
          change_dc==1 & direction_change ==0 ~ dr_cost * (100-input$percenchange1)/100,
          change_dc==0  ~ dr_cost),
  ...
  source("function_landscape_df.R")  #it sources the function that is used to create proportions of number of crops that will be cultivated
  ...
  df_landscape <- reactive({
    ...
    A <- A4 %>% 
      dplyr::mutate(prod_cost = other_variable_cost + fixed_cost +  fertilizer_cost + herb_insect_fung,
                    ...
                    wetland_interference = sample(0:1, num_row1, replace=T, prob=c(input$prop_margin, (1-input$prop_margin)))
      ) 
    ...
    C <- B %>% dplyr::mutate(
      yield = case_when(
        input$change_yd == "Yes" & input$direction_change == "increase"  ~ yield*(100+input$percenchange)/100,
        input$change_yd == "Yes" & input$direction_change == "decrease"  ~ yield*(100-input$percenchange)/100,
        input$change_yd == "No"  ~ yield),
      crop_price = case_when(
        input$change_pr == "Yes" & input$direction_change == "increase"  ~ crop_price*(100+input$percenchange)/100,
        input$change_pr == "Yes" & input$direction_change == "decrease"  ~ crop_price*(100-input$percenchange)/100,
        input$change_pr == "No"  ~ crop_price),
      upland_wtland_yd = case_when(  #This is where the yield differences between upland area and drained area is defined
      ...
      production_cost = case_when(
        input$change_pc == "Yes" & input$direction_change == "increase"  ~ production_cost*(100+input$percenchange)/100,
        input$change_pc == "Yes" & input$direction_change == "decrease"  ~ production_cost*(100-input$percenchange)/100,
        input$change_pc == "No"  ~ production_cost),
      #changepc = ifelse(change_yield == "Yes", 1, 0),
      ...
      drainage_cost = case_when(
        input$change_dc == "Yes" & input$direction_change == "increase"  ~ drainage_cost*(100+input$percenchange)/100,
        input$change_dc == "Yes" & input$direction_change == "decrease"  ~ drainage_cost*(100-input$percenchange)/100,
        input$change_dc == "No"  ~ drainage_cost),
      ...
      class_ab_wl = sample(1:0, num_row, replace=T, prob=c(input$prop_AB, (1-input$prop_AB)))
  ...
  #output$summary1 <- DT::renderDataTable({
    ...
    #mean_wlacre <- mean(df_landscape()$wetland_acre)
  #  mean_nc <- mean(df_landscape()$nuisance_cost)
  #  mean_nwl <- mean(df_landscape()$nuisance_cost)
  #  mean_pr <- mean(df_landscape()$crop_price)
  #  mean_yd <- mean(df_landscape()$crop_yield)
  #  mean_pc <- mean(df_landscape()$production_cost)
  #  mean_dc <- mean(df_landscape()$drainage_Cost)
  ...
  output$Wetland_policy_alberta <- DT::renderDataTable({
    ...
    total_wl_a <- as.integer(sum(df_class_a$wetland_acre))
    total_wcc_a <- as.integer(sum(df_class_a$npv_wl))
    total_restore_a <- as.integer((17700/2.471) * total_wl_a)
    policy_cost_a <- as.integer(abs(total_wcc_a - total_restore_a))
    
    #Class B Wetlands
    df_class_b <- df_landscape() %>% dplyr::filter(class_b_wl ==1) %>% dplyr::select(wetland_acre, npv_wl)
    total_wl_b <- as.integer(sum(df_class_b$wetland_acre))
    total_wcc_b <- as.integer(sum(df_class_b$npv_wl))
    total_restore_b <- as.integer((17700/2.471) * total_wl_b)
    policy_cost_b <- as.integer(abs(total_wcc_b - total_restore_b))
    
    #Class AB Wetlands
    df_class_ab <- df_landscape() %>% dplyr::filter(class_ab_wl ==1) %>% dplyr::select(wetland_acre, npv_wl)
    
    total_wl_ab <- as.integer(sum(df_class_ab$wetland_acre))
    total_wcc_ab <- as.integer(sum(df_class_ab$npv_wl))
    total_restore_ab <- as.integer((17700/2.471) * total_wl_ab)
    policy_cost_ab <- as.integer(abs(total_wcc_ab - total_restore_ab))
    
    final_df <- data.frame(
      Policy_Variables = c("Total Wetland Area (Acres)", "Total Conservation Cost ($)", "Total Restoration Cost ($)", "Cost of Wetland Policy ($)"),
      Class_A_Wetlands = c(total_wl_a, total_wcc_a, total_restore_a, policy_cost_a),
      Class_B_Wetlands = c(total_wl_b, total_wcc_b, total_restore_b, policy_cost_b),
      Class_AB_Wetlands = c(total_wl_ab, total_wcc_ab, total_restore_ab, policy_cost_ab)
    )
