...
#3. Uncertainties around annual net returns
  ...
  for (i in 1:sim) {
    ...
    df <- df %>% dplyr::mutate(
                        drainage_cost = dplyr::if_else(drainagecostsimulation1 ==1, unlist(pmap_dbl(list(n=1, dr_cost*0.3, dr_cost*1.3),runif)), dr_cost),
                        yield_sim = case_when(
                        ...
                        production_cost = dplyr::if_else(productioncostsimulation1==1, production_cost_sim, prod_cost),
